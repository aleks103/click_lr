	...
	<div class="text-center animated fadeInDown white-bg p-h-m">
		...
		<div class="panel panel-success m-t">
			...
			<div class="panel-body">
				<form class="form-horizontal" method="post" action="{{ route('groups.update', ['group' => $group->id]) }}">
					<div class="row">
						<div class="col-md-12">
							...
							<div class="form-group text-left">
								...
								<div class="col-xs-7">
									<input type="text" id="group_code" name="group_code" class="form-control" value="{{ $group->group_code }}"/>
								</div>
							</div>
							<div class="form-group text-left">
								...
								<div class="col-xs-7">
									<span class="radio m-r pull-left">
										<input type="radio" id="admin_no" name="allow_check" {{ $group->is_admin == 'no' ? 'checked' : '' }}/>
										<label for="admin_no">Not allowed</label>
									</span>
									<span class="radio pull-left">
										<input type="radio" id="admin_yes" name="allow_check" {{ $group->is_admin == 'yes' ? 'checked' : '' }}/>
					...
					<input type="hidden" id="is_admin" name="is_admin" value="{{ $group->is_admin }}"/>
					{{ method_field('PUT') }}
					{{ csrf_field() }}
				</form>
			</div>
