...
<html>
<head>
	@if($rotator_details['cloak_page_title'] != '')
		<title>{!! $rotator_details['cloak_page_title'] !!}</title>
	@endif
	@if($rotator_details['cloak_page_description'] != '')
		<meta name="description" content="{!! $rotator_details['cloak_page_description'] !!}"/>
	@endif
	...
	@if(count($magickbar_details) > 0)
		<script type="text/javascript">
			...
			var preview_url = BASE + "/tr/preview-html/" + html;
	...
	@if(count($timer_details) > 0)
		<script type="text/javascript">
			...
			$(function () {
				var timerCenter = (((window.screen.availWidth * 0.5 - 300) * 100) / (window.screen.availWidth * 1)) + "%";
				$foobar({
					"width": {
						"left": "0",
						"center": timerCenter,
	...
	@if(count($popup_details) > 0)
		<script type="text/javascript">
			...
			function previewPop() {
				if ($.cookie(cookie_name) === null) {
					setPopupCookie();
					var href = BASE + '/tr/load-popup-page/' + popup_id + '?tracking=1';
			...
			@if($popup_details->exit_method == 'Redir')
				if ($.cookie("psc16281") === null) {
					...
					$(window).load(function () {
						$(document).on('mouseout', function (event) {
							var event_target = event.relatedTarget || event.target;
...
<iframe id="myframe" class="clsIframeClass" src="{!! $rotator_details['track_url'] !!}" name="myframe" scrolling="yes" allowtransparency="true" marginheight="0" marginwidth="0"
        width="100%" height="100%" frameborder="0"></iframe>
@if($rotator_details['pixel_code'] != '')
	{!! $rotator_details['pixel_code'] !!}
@endif
